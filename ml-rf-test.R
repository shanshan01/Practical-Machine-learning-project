...
#
#pred3<-predict(model3,testing.new)
#Predright_prop3<-(sum(pred3==testing.new[,55])/nrow(testing.new))
#table(pred3,testing.new[,55])
...
pred.20
#set number of folds to 5 and 10,--final tuning parameters can not be determined


#reassign values outside the range
...
#no enough space to allocate-office computer
library(gbm)
model3<-train(training.new.trs$classe~.,
              method='gbm',data=training.new.trs, preProcess='pca',
              trControl = ctrl)
#final tuning parameters can not be determined
model4<-train(training.new.trs$classe~.,
              method='gbm',data=training.new.trs, preProcess=c("center","scale"),
              trControl = ctrl)


library(glmnet)
preProc<-preProcess(training.new.trs[,1:54],method='pca')
PC<-predict(preProc,training.new.trs[,1:54])
model5<-glmnet(training.new.trs[,55]~.,
               family='multinomial', data=PC)
...
ctrl <- trainControl(method = "repeatedcv",
                     repeats = 3)
model6<-train(training.new.trs[,55]~as.matrix(training.new.trs[,1:54]),
              method='glm',family='multinomial', preProcess='pca',
              trControl = ctrl)
